...
Ввод / явный ingest
        ↓
классификация утверждения + метаданные доказательств
        ↓
состояние Observed в L0/L1
        ↓
Guardian → TruthGate → проверки ограничений и противоречий
        ↓
физический многостатусный граф L3
...
Guardian + CanonicalView STRICT
        ↓
FactsPack + TRACE → ответ / отказ / Receipt

неразрешённое противоречие
        ↓
неизменяемый ContradictionReport
        ↓
авторизация actor/роли/области + decision lease
        ↓
явное решение куратора + причина
        ↓
аудируемый канонический путь записи

тематическая навигация
        ↓
рекомендательные метаданные TopicFacet
        ↓
только фильтрация/группировка — без допуска в Canon
```
