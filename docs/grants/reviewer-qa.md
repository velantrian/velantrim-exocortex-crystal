...
A separate owning Velantrim research repository, **Continuum**, has reached a frozen
preregistration for a bounded experiment comparing a competent current-state representation with
the **same current state plus a bounded transition trajectory**. The preregistration is frozen at
...
a protocol-frozen replaceable reader; the named provider model alias remains mutable and is not
weight-snapshot-pinned. Even a positive result would not prove that historical trajectory is
semantically necessary, that event sourcing is required, or that no better current-state-only
...
Continuum is not a submitted Crystal workstream and is not a candidate funded delta under the
current NLnet proposal.
...
for the full cross-project boundary and immutable source map.
