...
| CanonicalView (strict-grounding read boundary) | Partial | Yes | `core/canonical_view.py` — strict-grounding slice (`is_strict_canonical()`/`project_canonical()`) implemented and wired into `generate_answer()` (PR #257, corrective-hardened by PR #258 — 0 unresolved review threads on both); `review`/`full_graph` read modes, CLI/API `trusted_only` exposure, and conflicting-`VERIFIED`-facts abstention remain RFC-only (issue #220, `docs/CANONICAL_VIEW_RFC.md`) |
