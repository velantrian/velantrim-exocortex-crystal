# 🔱 Velantrim FractalMemory v8.0 "Crystal"
## Спецификация: Фрактальная Графовая Память для Автономного AI Агента

> **Версия**: 8.0 "Crystal" · **Дата**: Апрель 2026 · **Проект**: Velantrim ExoCortex
>
...
> Хирургический патч. Ядро не переписывалось. Обратная совместимость сохранена.

### 🔴 Багфиксы (критичные)
- **B1** `consume()` — добавлен `break` после `logger.critical` при исчерпании Redis retries (генератор зависал навсегда)
- **B2** `_persist_event_to_sqlite` — замена на HYPERIA-версию с ротацией `FALLBACK_MAX_ROWS=10_000` + `PRAGMA WAL` + устранён дубль dict-сборки
- **B4** `MemoryVolitionWorker` — добавлен `_initialized` guard: `write_voluntary()` теперь явно падает если `start()` не вызван (иначе `MAX_PER_SESSION` лимит молча не работал)

### 🟠 Несостыковки (исправлены)
- **N2** `invariant_test_coverage` → `100% (I1–I37) · I38–I65 pending` (было некорректно `100% I1–I65`)
- **N3** L4.5 — добавлено явное пояснение что RFC0052 + RFC0053 + RFC0065 живут на одном слое
- **N4** `ChatGPT-5.4` → `GPT-5.4` (верифицировано: модель реальная, март 2026)
- **N4b** DeepSeek V4 — убран из рабочей таблицы стека (`⚠️ не выпущен`), остался только в описании
- **N5** `BAE` → расшифрован как `Behaviour Anticipation Engine`

### 🟢 Оформление
- Убраны пробелы перед `:` в двух местах (`emotional_salience :` → `emotional_salience:`, `Drift Detection :` → `Drift Detection:`)

### 🆕 Перенесено из HYPERIA v5.20 (10 компонентов)
| # | Компонент | Файл | Суть |
|---|-----------|------|------|
| H-1 | **DAAD DomainResolver** | `memory/domain_resolver.py` | Domain-aware decay: `λ_eff = Σ(dᵢ×λᵢ)` · `active_project` живёт годами |
| H-2 | **Guardian** | `core/guardian.py` | Post-LLM валидатор: reject при conf<0.6, empty trace, all deprecated |
| H-3 | **ACT-R Activation** | `memory/actr_activation.py` | `B = ln(Σ tᵢ^-0.5)` · бонус в HybridRetriever по истории обращений |
| H-4 | **Laplace Confidence** | patch в `truth_gate.py` | `(pos+1)/(total+2)` · новые факты стартуют с 0.5, не блокируются |
| H-5 | **CognitiveModes** | `core/cognitive_modes.py` | PRECISION/BALANCED/EXPLORATION маршрутизатор retrieval |
| H-6 | **OutputFaithfulnessChecker F6.5** | `core/output_faithfulness_checker.py` | Slow Path: TF-IDF overlap ответа против L3 фактов |
| H-7 | **MemoryBudgetPlanner** | `memory/memory_budget_planner.py` | 500k hard limit · auto-GC при 85% (425k) |
| H-8 | **CircuitBreaker** | `circuit_breaker.py` | per-loop asyncio.Lock · Neo4j/Redis/LLM cascading failures |
| H-9 | **SOARGoalNode** | patch в `memory/core_memory_blocks.py` | `GoalNode(priority, parent_id)` · иерархия целей |
| H-10 | **Cascade Strategy Invalidation** | patch в L4 GC Worker | Факт Deprecated → Strategy немедленно `valid=false` |

### Новый инвариант
- **I66**: DAAD меняет только `attention_weight`. `epistemic_state`/`truth_status` — неприкосновенны.
...
### 🔴 Багфиксы и несостыковки
- **P1** Двойной разделитель — убран один `---` (строки 48–50 changelog)
- **P2** Устаревшая версия — `"Velantrim v5 умеет помнить..."` → историческая ссылка через RFC0065
- **P3** Инвариант `I74` — добавлен: L2.5 Staging на read-пути только `preliminary × 0.7`
- **P4** Инвариант `I75` — добавлен: именование ProtoConcept ТОЛЬКО в Slow Path
- **P5** `goal_alignment = 0.5` — добавлено предупреждение про w_goal=1.2
- **P6** `SBEAsyncWorker` — уточнён триггер запуска через EventBus (`ANALOGY_CANDIDATE_READY`)
- **P7** Roadmap инвариантов I38–I65 — добавлена таблица фаз

### 🟢 Аддитивные дополнения
- **A1** HyDE opt-in — конфиг-флаг `hyde_enabled: false` + механизм + инвариант
- **A2** TraversalPolicy — стратегия обхода рёбер по типу запроса + I76
- **A3** LateralInhibition — защита от Hub Explosion + I77
- **A4** RFC0068 NeuroCore — новый раздел Phase 0 Draft

### 🟡 Дополнительные находки (из аудита)
- **E1** Метрика `reasoning_bank_ucb1_score` в таблице метрик — добавлена пометка `[DEPRECATED]` + `TODO: remove in v8.0`

---

## 📋 Changelog v7.5 "Titan" → v8.0 "Crystal"

> Хирургический патч. Ядро не переписывалось. Обратная совместимость сохранена.
> Источник: глубокий аудит Opus + консенсус 5 ИИ + 3 research papers (SYNAPSE, MAGMA, FadeMem).
> Валидация: архитектура подтверждена тремя независимыми papers января 2026.

### Почему "Crystal"
Название отражает переход от чисто фрактальной иерархии к кристаллизованной структуре знаний:
каждый факт = атомарный "кристалл" с типом, версией, провенансом и FSRS-состоянием.
Три новых оси: когнитивная типизация (EPISODIC/SEMANTIC/PROCEDURAL),
knowledge_type (TERM/FACT/LAW/MODEL/METHOD/OPINION), per-node versioning (_version_).

### 🔴 P0 — Критические улучшения (3 патча)
- **P0-1** FSRS decay заменяет Ebbinghaus: `R = (1 + 19/81 × t/S)^(-0.5)` вместо `R = e^(-t/S)`. Точнее на 20-30% (FadeMem paper). Новый модуль `memory/fsrs_state.py`. Поля difficulty/stability/retrievability в MemoryItem.
- **P0-2** D-Mem Quality Gating: новый метод `quality_gate()` в Guardian. Экономия ~60% токенов при 96.7% качества (D-Mem paper). Маршрутизация Fast ↔ Slow Path по confidence × coverage × contradictions.
- **P0-3** Qwen3 Reranker fallback: ColBERTv2 / bge-reranker-large как default. Qwen3 — только opt-in с native Transformers (known issues в vLLM/llama.cpp).

### 🟠 P1 — Структурные расширения (7 патчей)
- **P1-1** MAGMA multi-graph: 4 типа рёбер (SEMANTIC_REL / TEMPORAL_REL / CAUSAL_REL / ENTITY_REL) + IntentRouter в `memory/intent_router.py`. I86.
- **P1-2** Когнитивная типизация: `MemoryType` (EPISODIC/SEMANTIC/PROCEDURAL) — новая ортогональная ось поверх L0–L7.
- **P1-3** `knowledge_type` на :Fact (TERM/FACT/LAW/MODEL/METHOD/CONSTRAINT/OPINION). Ортогонально ESM. I87.
- **P1-4** Per-node versioning: `_version_: int` + OCC Cypher в ESM-переходах. I88.
- **P1-5** Provenance chain: `provenance_chain: List[Dict]` вместо одного `source_type`. Append-only. I89.
- **P1-6** HippoRAG Personalized PageRank в HybridRetriever: +7% на ассоциативных задачах, 0 LLM-вызовов. `memory/pagerank.py`.
- **P1-7** Inverted HyDE offline: генерация гипотетических запросов к важным эпизодам в SleepTimeWorker. I90.

### 🟡 P2 — Улучшения (7 патчей)
- **P2-1** Fan-effect dampening в Velum: ACT-R fan-effect `1/log(degree+1)` в `_strengthen_edge`.
- **P2-2** Graph Health Checker: раз в сутки через SleepTimeWorker — orphans, dupes, fan-out violations.
- **P2-3** Conflict Resolution Window 60 сек: после "я ошибся" TruthGate снижает барьер для user_input.
- **P2-4** Atomic Split в ингестии: один смысл = один узел, перед TruthGate. I91.
- **P2-5** ReasonGraph DAG в CausalGraph: мини-DAG рассуждения в Slow Path ([:SUPPORTS]/[:CONTRADICTS]). I95.
- **P2-6** Curiosity Engine: система инициирует вопросы при gap < 3 фактов. Только Slow Path. I92.
- **P2-7** Trace Examples: эталоны мышления (intent→evidence→truth_class→policy→action) как :TraceExample узлы. I93.

### 🆕 Новые инварианты I84–I95 (12 шт.)
- **I84** FSRSIsolation · **I85** QualityGate · **I86** IntentRouter · **I87** KnowledgeTypeImmutable
- **I88** VersionOCC · **I89** ProvenanceAppendOnly · **I90** InvertedHyDE · **I91** AtomicSplit
- **I92** CuriositySlowOnly · **I93** TraceExampleReadOnly · **I94** LadybugDBCompat · **I95** ReasonGraphDAG

### 🆕 Новый backend: LadybugDB
- Community-форк KuzuDB (после покупки Kuzu компанией Apple в 2025). MIT, Cypher, columnar, ACID.
- Полная Kuzu API совместимость — миграция без потери данных. I94.
- Заменяет KuzuDB в конфигурации Personal.
...
**Truth Gate + ESM.** Ни один факт не попадает в L3 граф без прохождения через Truth Gate. Каждый факт живёт в одном из семи эпистемических состояний (ESM): Hypothesized → Supported → Validated → ImmutableCore или Contradicted → Deprecated. Переходы — только через ESM.transition(), прямой SET epistemic_state — баг.
...
> **P2-4 (v8.0):** После извлечения факта, ПЕРЕД TruthGate — вызывается `atomic_split()`:
...
| Диапазон | Область | Целевая версия |
|----------|------------------------------------|----------------|
| I38–I45 | RFC0054 SAE инварианты | v7.1 |
| I46–I52 | RFC0055–0057 эпистемика | v7.1 |
| I53–I58 | RFC0058–0061 безопасность | v7.2 |
| I59–I65 | RFC0063 Knowledge Ingestion | v7.2 |
...
│    → Quality Gate (confidence × coverage × contradictions)  │  ← P0-2
...
L0: Рабочая память (Working Memory)
    ...
    │
    ├─ CoreMemoryBlocks — постоянный профиль пользователя  ← из HYPERIA
...
L1.5: Velum — Synaptic Pre-Graph Layer + Salience Detector   ← RFC0016
    ...
    │   под _edges_lock (asyncio.Lock Velum).
    │   Нарушение: изменение весов рёбер при LateralInhibition без _edges_lock.
...
class ProtoConcept:
    ...
    def update_confidence(self):
        # Консервативная формула. Максимум при co_occur=20 + cross=10 -> 0.83
        self.confidence = (min(1.0, self.co_occur_count / 20.0) * 0.6 +
...
class ConceptEmergenceDetector:
    ...
    def __init__(self, db, truth_gate=None, llm_client=None):
        self.db, self.truth_gate, self.llm_client = db, truth_gate, llm_client
    ...
    def observe(self, entities: list[str], session_id: str):
        # I50-c: только счётчики, не содержимое эпизода.
        # лимит MAX_ENTITIES защищает event loop от O(N!) combinations
        if len(entities) < self.MIN_ENTITIES:
            return
        entities = entities[:self.MAX_ENTITIES]  # лимит до combinations
        from itertools import combinations
        for size in range(self.MIN_ENTITIES, len(entities) + 1):
            for combo in combinations(sorted(entities), size):
                ...
                if (self._matrix[key]          >= self.CO_OCCUR_MIN and
                        len(self._sessions[key]) >= self.CROSS_SESSION):
    ...
    async def _tfidf_name(self, proto: ProtoConcept) -> str:
        # Извлечение общего корня из имён сущностей — 0 токенов
        entity_words = [e.lower().replace("_", " ").split() for e in proto.entities]
    ...
    async def gc_expired(self):
        # Очищаем _entity_to_protos от expired proto_ids
        now, ttl = datetime.now(timezone.utc), timedelta(days=self.TTL_DAYS)
        expired_ids = []
        for proto in self._protos.values():
        ...
        if expired_ids:
            logger.info(f"ConceptEmergence GC: {len(expired_ids)} proto expired")
```
...
# I50: ConceptEmergenceDetector не пишет в граф
async def test_I50_concept_emergence_no_graph_writes():
    graph    = MockGraph()
    detector = ConceptEmergenceDetector(db=MockDB(), truth_gate=None)
    for i in range(10):
        detector.observe(["A", "B", "C"], session_id=f"s{i}")
    assert graph.write_count == 0, "I50 VIOLATION: пишет в граф"
...
async def test_I50b_proto_promote_requires_truthgate():
    ...
    p    = ProtoConcept(proto_id="proto:t01",
                        entities=frozenset(["A","B","C"]),
                        co_occur_count=7, cross_sessions=4,
                        name="test", confidence=0.75)
...
class EmergenceConfig:
    ...
    CROSS_SESSION     = 3  # FIX: переименовано с CROSS_SESSION_MIN → CROSS_SESSION
                           # чтобы совпадать с ConceptEmergenceDetector.CROSS_SESSION
                           # и не вызывать AttributeError при self.CROSS_SESSION
    MIN_ENTITIES      = 3
    MAX_ENTITIES      = 7
    NAMING_THRESHOLD  = 0.7
    TTL_DAYS          = 30
...
L2: Среднесрочная память (Medium-Term Memory)           ← RFC0013
    ...
    └─ Decay: FSRS power-law (v8.0 — заменяет Ebbinghaus экспоненту, P0-1)
         ...
         # БЫЛО (v7.5): R = e^(-t/S)  — заменено согласно P0-1
...
L3: Долгосрочная память (Long-Term Memory)
    ...
    ├─ P1-3: knowledge_type на :Fact (v8.0):                     ← P1-3
    ...
    │
    ├─ P1-5: Provenance Chain (v8.0):                             ← P1-5
    ...
    ├─ P1-1: Multi-Graph Decomposition (MAGMA-style):             ← P1-1
    ...
    ├─ P1-4: Per-node Versioning (OCC):                           ← P1-4
...
L4: Reasoning Layer — Самообучение на опыте          
    ...
    │   · reasoning_bank_ucb1_score — ⚠️ [DEPRECATED] alias → reasoning_bank_ts_score
    │     (обратная совместимость только · TODO: remove in v8.0)
...
Все предыдущие слои памяти работают **пассивно**: система наблюдает и решает что запомнить. RFC0065 добавляет агенту **право голоса в собственной памяти**: через tool call `memory.write_voluntary()` агент принимает осознанное решение записать факт в L3 — без ожидания пассивной консолидации.
    ...
    ├─ P2-5: ReasonGraph DAG (v8.0):                              ← P2-5
    ...
    │
    ├─ P2-6: Curiosity Engine (v8.0):                             ← P2-6
...
class MemoryVolitionWorker:
    ...
    async def process_event(self, event: VolitionEvent) -> VolitionResult:
        ...
        # emotional_salience=0.0, не importance_hint.
        # importance_hint != emotional_salience.
        # Использование hint как прокси случайно вызывает ESM.freeze().
        gate = await self.truth_gate.validate_and_transition({
            "id": staged.id, "content": event.content,
            "confidence": event.importance_hint,
...
| **Personal** | LadybugDB | FAISS | Локальный ПК, офлайн, MIT · I94 |
| **Startup** | FalkorDB | Qdrant | Продакшн, стартап |
| **Enterprise** | Neo4j | Qdrant | Корпорации, RBAC, GDPR |

```python
# velantrim_config.py
GRAPH_BACKEND = "ladybugdb"  # "ladybugdb" | "neo4j" | "falkordb" | "sqlite"
# LadybugDB — https://ladybugdb.com · MIT · Cypher · columnar · vector + full-text · ACID
# I94 (LadybugDBCompat): LadybugDB backend совместим с Kuzu API. Миграция без потери данных.
```
...
| **KuzuDB** | Куплен Apple в 2025 — заменён на LadybugDB (community-форк, MIT, полная совместимость) |
...
class RobustEventBus:
    ...
    def __init__(
        ...
        max_retries: int = 3,
        config: dict = None,   # ← FIX-A: config был неопределён — NameError при старте
    ):
        ...
        self.fallback_queue = SQLiteFallbackQueue(
            db_path=_cfg.get("fallback_db", "fallback_events.db")  # ← FIX-A
        )
    ...
    async def _persist_event_to_sqlite(self, event: AgentEvent):
        ...
        FIX-B2 (HYPERIA): добавлена ротация FALLBACK_MAX_ROWS=10_000 + PRAGMA WAL/NORMAL
        ...
        while True:
            try:
                ...
                if failed_count >= max_failures:
                    logger.critical("Redis unavailable, entering degraded mode")
                    break  # ← выходим из while True — иначе генератор зависает навсегда
            ...
            for stream, message_list in messages:
                for msg_id, data in message_list:
                    ...
                    if retry_count < 3:
                        ...
                        await self.redis.xdel(self.dlq_key, msg_id)  # ← FIX-B1: удалять ПОСЛЕ успешного reprocess, а не в except
                    else:
                        ...
                        await self.redis.xdel(self.dlq_key, msg_id)  # ← FIX-B2: удалять после архивирования
        except Exception as e:
    ...
    async def _archive_permanent_failure(self, msg_id: str, data: dict):
        ...
        # FIX-B3: убран orphaned except (был SyntaxError — except без try),
...
## 🧬 Компоненты перенесённые из HYPERIA v5.20

> Хирургический перенос проверенных компонентов HYPERIA в Velantrim без изменения архитектуры.
> Каждый компонент — отдельный файл. Интегрируются через существующие точки подключения.

---

### HYPERIA-1: DAAD — Domain-Aware Attention & Decay

> **Источник**: HYPERIA V5.20 ULTIMATE (Patch DAAD — главный архитектурный патч)
> **Проблема**: Velantrim использует DAAD domain-aware λ_eff в FSRS decay для всех узлов (v8.0).
...
# λ_eff используется вместо глобального λ
decay_factor = exp(-t / max(0.01, S / params.lambda_eff))
new_importance = max(params.floor_eff, current_importance * decay_factor)
...
async def invalidate_stale_strategies(graph) -> int:
    ...
    HYPERIA FIX: confidence × 0.5 создавал зомби-стратегии (0.95 → 0.475 → никогда не удалялась).
...
class FractalMemory:
    def __init__(self, graph_memory: GraphMemory, llm_client=None):
        ...
        self.stm_decay_rate = 0.1
        self.mtm_decay_rate = 0.05
        self.ltm_decay_rate = 0.01
    ...
    async def apply_decay(self) -> dict:
        ...
        async with self._cache_lock:
            ...
            for item in self.stm_cache:
                ...
                item.importance = float(np.clip(
                    item.importance * np.exp(-age_h / (strength * 24)), 0.0, 1.0
                ))
                stats["stm_decayed"] += 1
            ...
            for item in self.mtm_cache:
                ...
                strength = max(0.01, item.importance * (1.0 + np.log1p(item.access_count)))
                item.importance = float(np.clip(
                    item.importance * np.exp(-age_h / (strength * 168)), 0.0, 1.0
                ))
                stats["mtm_decayed"] += 1
    ...
    def _calculate_importance_with_decay(
        ...
        - Temporal decay (экспоненциальный)
        ...
        """
        # 1. Базовый temporal decay
        temporal_decay = np.exp(-self.stm_decay_rate * age_hours)
...
def personalized_pagerank(
    ...
    personalization = {n: 1.0 / len(query_nodes) for n in query_nodes if n in G}
...
class ReasoningBank:
    ...
    async def distill_strategies(self):
        ...
        """
        try:
            # Группировать опыт по типу задачи
            grouped = self._group_by_task_type(self.experience_buffer)
            ...
            # Очистить буфер только после успеха
            self.experience_buffer.clear()
        except Exception as e:
            logger.error(
                f"distill_strategies failed, buffer preserved "
                f"({len(self.experience_buffer)} items): {e}"
            )
    ...
    async def ace_curator_update(self):
        ...
        Дистилляция стратегий с structured reasoning: root_cause + conditions + anti_conditions.
        
        Вызывается ТОЛЬКО из SleepTimeWorker в idle — не из Fast Path.
        
        ✅ Исправление из HYPERIA: JSON парсинг через find/rfind вместо regex
        (regex ломался на вложенных массивах в строках JSON).
        """
        if not self.llm_client:
            logger.debug("ACE Curator: llm_client=None, пропускаем")
            return
        
        try:
            # Берём топ-5 стратегий по confidence
            top = sorted(
            ...
            # Последние 10 опытов
            recent = list(self.experience_buffer)[-10:] if hasattr(self, 'experience_buffer') else []
            
            prompt = f"""Analyze strategies and recent experiences.
Return ONLY JSON array (no other text):
[{{"id":"prefix","delta":0.1,"root_cause":"why it works",
   "conditions":"when to apply","anti_conditions":"when NOT to apply"}}]
...
Experiences: {[{
    'task':e.task_description[:50],
    'outcome':e.outcome.value
} for e in recent]}"""
            
            # Проверка доступности графа (ping)
            await self.graph.execute_cypher("RETURN 1", {})
            
            # LLM вызов
            resp = await self.llm_client.complete(prompt)
            
            # ✅ Исправление из HYPERIA: find/rfind вместо regex
            _start = resp.find('[')
            _end = resp.rfind(']')
            
            if _start == -1 or _end == -1 or _end <= _start:
                logger.warning("ACE Curator: LLM не вернул JSON-массив")
                return
            
            json_str = resp[_start:_end+1]
            
            import json
            try:
                updates = json.loads(json_str)
            except json.JSONDecodeError as e:
                logger.warning(f"ACE Curator: JSON parse error: {e}")
                return
            
            # Применяем обновления
            for upd in updates:
                strategy_id_prefix = upd.get("id", "")
                s = next(
                    (x for x in self.strategies.values() if x.id.startswith(strategy_id_prefix)),
                    None
                )
                ...
                # Сохраняем reasoning в граф
                await self.graph.add_episode(
                    episode_name=f"ace_curator_{s.id[:8]}_{datetime.now(timezone.utc).isoformat()}",
                    content=json.dumps({
                        "strategy_id": s.id,
                        "root_cause": upd.get("root_cause", ""),
                        "conditions": upd.get("conditions", ""),
                        "anti_conditions": upd.get("anti_conditions", ""),
                        "delta": delta,
                        "curator": "ace_v1"
                    }, ensure_ascii=False),
                    source="ace_curator"
                ...
                await self._save_strategy(s)
            
            logger.info(f"ACE Curator: обновлено {len(updates)} стратегий")
            
        except Exception as e:
            logger.warning(f"ace_curator_update failed (non-fatal): {e}")
```
...
class Velum:
    ...
    async def _update_edge(self, entity_a: str, entity_b: str,
        ...
        async with self._lock:
            ...
            if (edge.weight >= MEMORY.VELUM_CO_OCCUR_THRESHOLD / 10
    ...
    def _gc_weak_edges(self, keep_ratio: float = 0.75):
        """
        GC слабых рёбер co-occurrence. ← из HYPERIA
    ...
    def _load_seed_from_sqlite(self):
        """
        Восстановить топ рёбра из предыдущей сессии. ← из HYPERIA
    ...
    async def _save_top_edges_to_sqlite(self, top_n: int = 200):
        """
        Сохранить топ-N рёбер по весу в SQLite. ← из HYPERIA
...
> ⚠️ **Каноническая реализация — FEATURE-8 (RFC0062).** Актуальный код см. в разделе RFC0062 · FEATURE-8.
```
...
class AutonomousSelfLearningAgent:
    ...
    async def _graceful_shutdown(self):
        ...
        snapshot = {
            ...
            # L0 Working Memory — только CRITICAL/HIGH (Ring Zero + активная цель)
            "working_memory": [
...
class ObservableHybridRetriever(HybridRetriever):
    ...
    async def retrieve(
        ...
        with tracer.start_as_current_span("stm_search") as span:
            if strategy in ["conversation", "immediate"]:
...
class MemoryGarbageCollector:
    ...
    async def _merge_duplicate_entities(self) -> int:
        ...
        ⚠️ APOC-зависимость нарушает backend-агностику.
        apoc.refactor.mergeNodes работает ТОЛЬКО с Neo4j + APOC плагином.
        При использовании FalkorDB / Kuzu / Graph-Lite — GC упадёт с ошибкой.
        Для non-Neo4j бэкендов используй fallback ниже (стандартный Cypher).
        """
        # ⚠️ APOC path — только для Neo4j с подключённым APOC плагином
        # (NEO4J_PLUGINS=["apoc"] в docker-compose)
        ...
        for pair in candidates:
            ...
            # MERGE (e1)-[r2:SAME_TYPE] хардкодил тип связи — все оригинальные
            # рёбра (:RELATED_TO, :CONCEPT_OF и др.) уничтожались. Заменено на apoc.refactor.mergeNodes
            # (APOC подключён в docker-compose: NEO4J_PLUGINS=["apoc"])
            merge_query = """
            ...
            await self.graph.execute_cypher(merge_query, {
                "e1_id": e1["id"],
                "e2_id": e2["id"]
            })
    ...
    async def _cascade_invalidate_dependent_strategies(self) -> int:
        ...
        s.is_active = CASE
            ...
            ELSE true
        END,
        s.penalized_fact_ids = coalesce(s.penalized_fact_ids, []) + new_invalid_ids
...
# === Strategy Learning ===
strategy_ucb_scores = Histogram(
    "strategy_ucb_score",
    "UCB1 scores for strategy selection",
    buckets=[0, 0.2, 0.5, 0.8, 1.0, 2.0, 5.0]
)
...
## 🔌 MCP Server — Подключение к внешним клиентам ← из HYPERIA
...
class ImmutableRawMemory:
    ...
    def _init_schema(self):
        """Создать таблицу — append-only, нет UPDATE/DELETE"""
        import sqlite3
    ...
    def save_episode(self, episode_id: str, content: str,
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                # PATCH-7: явные имена колонок — защита от тихого сломания при изменении схемы
...
## 🔗 CausalGraph — Слой причинно-следственных связей ← из HYPERIA
...
```python
# memory/causal_graph.py  ← из HYPERIA, адаптирован для Velantrim
...
RFC0067 v2.0: Creative Intelligence Layer
    ...
    |   I51-I54: зарезервированы (RFC0067 v1.0 устарела).
    |
    +- Метрики:
...
class ResonanceTracker:
    ...
    async def _get(self, aid, stype):
        if stype == "analogy_graph":
            ...
            result = await self.graph.execute_cypher(
                "MATCH ()-[r:METAPHOR_OF|ANALOGOUS_TO]->() "
                "WHERE id(r)=$id RETURN r.resonance_score AS s",
                {"id": aid})
...
class AdaptiveDecoder:
    ...
    MODE_TEMPS = {"PRECISION": 0.1, "BALANCED": 0.5, "EXPLORATION": 0.7}
...
Структура:
  migrations/
    v5_01_add_evidence_count.cypher
    v5_02_add_hypothesis_node.cypher
    v5_03_add_etir_weights.cypher
    v5_04_add_session_id_l1.cypher
    v5_05_add_cognitive_mode_and_budget.cypher
...
Поле SchemaVersion в Neo4j:
  CREATE (m:SchemaVersion {version: "5.0", applied_at: datetime()})
```

Миграция (`migrations/v5_05_add_cognitive_mode_and_budget.cypher`):
...
MERGE (m:SchemaVersion {version: "5.0"})
...
SET n.activation = n.activation * exp(-0.18)
...
class EtirEngine:
    ...
    CYPHER_SPREAD  = """
        ...
        SET m.activation = coalesce(m.activation, 0.0) + incoming
    """
    CYPHER_DECAY   = "MATCH (n) WHERE n.activation IS NOT NULL AND n.is_ring_zero <> true SET n.activation = n.activation * exp(-0.18)"
    ...
    async def activate(self, query: str, seed_ids: List[str]) -> ActivationResult:
        ...
        async with self.driver.session() as session:
            try:
                ...
                # μ=0.07 из RFC0011. Только если APOC доступен (community edition).
                try:
                    await session.run(
                        self.CYPHER_INHIBIT,
...
class WeightedSemanticDecay:
    ...
    async def _apply_decay(self, node_id: str, penalty: float):
        ...
        SET n.importance = n.importance - $penalty,
...
class SemanticQuarantine:
    ...
    async def review_quarantine(self):
        ...
        RETURN n.id, n.importance_score
        """
...
class FractalSimilarityMonitor:
    ...
    def _correlation_dimension(self, snapshot) -> float:
        ...
        """
        # Извлечь embeddings всех узлов из snapshot
        embeddings = snapshot['node_embeddings']  # numpy array [N, dim]
        ...
        for r in radii:
            ...
            C_r.append(count / (len(embeddings) * (len(embeddings) - 1)))
...
|---|---|
| **LadybugDB** | ☠️ Не существует. Галлюцинация DeepSeek-ответа. Нет GitHub, нет кода, нет ничего |
...
> **Урок из LadybugDB**: всегда верифицировать названия инструментов перед включением в архитектуру. Любой разработчик загуглит и не найдёт — документ потеряет доверие.
...
### 🔢 EmbeddingRegistry — защита от смешивания размерностей ← из HYPERIA
...
## 🔧 Обслуживание системы ← из HYPERIA
...
async def merge_group(graph: GraphMemory, nodes: list, dry_run: bool):
    ...
    for dup in duplicates:
        # Перенести входящие и исходящие рёбра
        await graph.execute_cypher("""
            MATCH (dup:Entity {id: $dup_id})-[r]->(target)
            WHERE dup.id <> $canonical_id
            MERGE (canon:Entity {id: $canonical_id})-[nr:RELATED_TO]->(target)
            SET nr.weight = coalesce(nr.weight, 0) + coalesce(r.weight, 0.5)
            DELETE r
        ...
        await graph.execute_cypher("""
            ...
            WHERE dup.id <> $canonical_id
            MERGE (source)-[nr:RELATED_TO]->(canon:Entity {id: $canonical_id})
            SET nr.weight = coalesce(nr.weight, 0) + coalesce(r.weight, 0.5)
            DELETE r
        """, {"dup_id": dup["id"], "canonical_id": canonical["id"]})
...
# GRAPH_BACKEND = "ladybugdb"  # v8.0: LadybugDB — community-форк KuzuDB (MIT, Cypher, ACID)
...
class FactRouter:
    ...
    PATTERNS = {
        ...
        QueryType.CHAT     : ["привет", "спасибо", "пока",  # дубль "привет" заменён на "пока"
                               "hello", "thanks", "как дела"],
    }
    ...
    def route(self, query: str) -> RouteDecision:
        ...
        # Нет паттернов → CHAT или FACT по умолчанию
...
reasoning_bank_ucb1_score        # alias → reasoning_bank_ts_score (совместимость)
...
GRAPH_BACKEND          = {"weak": "graph_lite", "medium": "ladybugdb", "strong": "neo4j"}[_HW]
...
|-------------|------|--------|--------|
| Graph DB | SQLite Graph-Lite | LadybugDB embedded | Neo4j 5.26+ |
...
```python
# nlp_utils.py — синглтон MorphAnalyzer + кэш нормализации  ← из HYPERIA
...
Kuzu     → добавить GRAPH_BACKEND в velantrim_config.py
...
RFC0039  → обновить reasoning_bank.py: заменить UCB1 на Thompson Sampling
           ...
           + добавить alias метрику reasoning_bank_ucb1_score
...
# ── I77: LateralInhibition под _edges_lock ────────────────────────────────────
async def test_I77_lateral_inhibition_under_edges_lock():
    """I77: LateralInhibition выполняется строго под _edges_lock Velum."""
    # TODO: проверить что _edges_lock захвачен при вызове _strengthen_edge
    pass  # stub — реализовать в v7.1
...
async def test_I94_ladybugdb_compat():
    """I94 (LadybugDBCompat): LadybugDB backend совместим с Kuzu API. Миграция без потери данных."""
    # TODO: прогнать Kuzu API тесты против LadybugDB адаптера
    pass  # stub
