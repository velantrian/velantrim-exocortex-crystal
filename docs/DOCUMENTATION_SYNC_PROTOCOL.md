...
| English README | Primary public source presentation | Must not overstate implementation |
| CURRENT localized README | Full translated public presentation | Must remain semantically aligned with English evidence |
| ORIENTATION_ONLY / REFRESH_NEEDED translations | Navigation or incomplete translation | Never override current evidence |
| `docs/ai/*` | Compact state, component map, risks, audit method and hand-off | GitHub-native continuity for humans and AI agents |
...
Notion can preserve detailed explanations that would overload the public repository. GitHub
must still contain enough technical and audit information for an external reviewer or
connectorless AI agent to understand, test, continue and challenge the work without access
...
This does not require copying every sentence twice. Synchronize the decision-bearing facts:
...
Notion remains the richer rationale and historical layer. GitHub remains the complete public
technical and audit layer.
...
- a durable change to documentation language, localization scope or translation governance.
...
A missing connector is normally `HANDOFF_REQUIRED`, not a reason to abandon the analysis or
hide its findings.
...
2. Read `docs/LOCALIZATION_POLICY.md` and `docs/TRANSLATION_STATUS.md` when README, public
   wording or translations may change.
3. For `GITHUB_AND_NOTION`, read the related Project Hub record when access exists.
4. When Notion access does not exist, inspect `docs/ai/NOTION_HANDOFF.md` for pending items
   and continue from GitHub evidence.
5. Verify the exact `main` baseline and keep research/roadmap claims separate from
...
4. Record material analysis in GitHub even when it does not immediately change runtime:
   ...
   surface.
5. For an English implementation PR, record whether public translated meaning changed.
6. Keep broad translation work in a separately reviewable documentation-only PR unless the
   translation change is tiny and unambiguous.
...
1. Update every affected GitHub surface, which may include:
   ...
   - one or more localized README or document families in a dedicated translation PR;
   - `docs/LOCALIZATION_POLICY.md` and `docs/TRANSLATION_STATUS.md` when phase/status changes;
   - `TEST_REPORT.md`;
   - `docs/STATUS.md`;
...
5. Set synchronization to `SYNCED` before a `GITHUB_AND_NOTION` implementation PR is marked
   ready for review.
...
3. Add a structured item to [`docs/ai/NOTION_HANDOFF.md`](./ai/NOTION_HANDOFF.md) containing
   the problem, findings, decision, alternatives, boundaries, changed files, exact SHA/CI
   evidence, limitations, intended Notion record title and next actions.
4. Set PR fields to:
...
6. Keep a `GITHUB_AND_NOTION` implementation PR draft until a connected actor completes
   synchronization. Analysis-only findings may remain in the queue without blocking unrelated
   engineering, but the hand-off must remain visible.
...
6. after merge, add final merge SHA, CI evidence, deviations, limitations and next actions
   to Notion.
...
- translation phase, source checkpoint and freshness-state changes, if any.

Also close or mark the corresponding GitHub hand-off item `SYNCED` so agents without Notion
can see that synchronization was completed.
...
verified code and tests
  ...
  + correct translation status and source checkpoint when localization changes
...
A Notion plan is not implementation proof. A merged PR with stale status or public docs is
not complete. A connectorless analysis hidden only in chat is not a durable hand-off. A
checked box without the corresponding record is not synchronization.

## 9. Active language and localization policy

English is the primary working, source and conflict-resolving GitHub documentation language.
English-first means source-first, not English-only.

### Root README target

Every supported `README.<locale>.md` targets full visual and semantic parity with the English
`README.md`, including project positioning, meaningful emoji, mind maps, ASCII diagrams,
architecture trees, tables, verified evidence, limitations, quick start and navigation.
A permanent short-summary model is not acceptable.

### Progressive document translation

Translation is phased rather than all-at-once:

1. merge and verify the English implementation/documentation baseline;
2. assess which translated public meaning changed;
3. open one or more dedicated docs-only PRs for a language or document family;
4. record the exact English source checkpoint and freshness status;
5. validate local links, claim boundaries and natural-language fidelity;
6. merge completed phases without waiting for every supported language or the whole corpus.

`ORIENTATION_ONLY` is temporary. Existing translated files are `REFRESH_NEEDED` until checked
against a recorded source checkpoint. A translation may not strengthen English capability,
security, grant or authority claims.

The active policy and rollout are maintained in
[`LOCALIZATION_POLICY.md`](./LOCALIZATION_POLICY.md) and
[`TRANSLATION_STATUS.md`](./TRANSLATION_STATUS.md). The `docs-status` job verifies objective
invariants without imposing a maximum README size or pretending to replace semantic language
review.
