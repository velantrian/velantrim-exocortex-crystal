...
| **Epigenetic Adaptation** | Implemented (telemetry/research) | A single process-local verification tag is raised by gate blocks and relaxed by successful answers. `core/adaptation.py` exposes the resulting `verification_threshold()` as an advisory signal, but the default TruthGate admission threshold is fixed and versioned in `core/truth_gate.py` and does not consume adaptive history. The mechanism never rewrites verified facts or silently changes default admission authority (RFC0071). | High-level metaphor of contextual regulation | Not biological epigenetics; does not mutate truth; not per-fact metadata weighting; adaptation telemetry is not admission authority |
