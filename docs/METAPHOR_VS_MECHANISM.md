...
| **Epigenetic Adaptation** | Implemented (telemetry/research) | Gate blocks raise a process-local verification tag. Successful answers record low-stress telemetry; in the current implementation that increases exploration/creativity but does not lower the verification tag. `core/adaptation.py` exposes `verification_threshold()` as an advisory signal, while the default TruthGate admission threshold is fixed and versioned in `core/truth_gate.py` and does not consume adaptive history. The mechanism never rewrites verified facts or silently changes default admission authority (RFC0071). | High-level metaphor of contextual regulation | Not biological epigenetics; does not mutate truth; not per-fact metadata weighting; adaptation telemetry is not admission authority |
