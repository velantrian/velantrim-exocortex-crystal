...
| Threat | Mitigation in code |
|--------|--------------------|
| Direct L3 write bypassing the gate | **Primary entry point**: admission / ingest writes go through `truth_gate()`; `store_fact` writes L0/L1 only, never L3. Secondary sync paths (`reconcile._sync_l3()`, `compliance._sync_restriction()`, `drain_l3_outbox()`) skip re-gating but **block `Observed` facts** — only post-admission states (e.g. `Validated`, `Contradicted`, `Deprecated`) or post-gate outbox entries are merged. Residual: sync paths still do not re-run TruthGate on metadata changes. |
