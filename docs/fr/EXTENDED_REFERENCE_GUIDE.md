...
# Référence étendue Crystal

D5 distingue `CURRENT`, `RETIRED` et `ENGLISH_ONLY_BY_DESIGN`. physical L3 != strict Canon; retrieval score != evidence; model output != source truth; migration proof != claim proof; import success != activation. PostgreSQL `active=false`.
...
RC-1/RC-2 sont des fondations bornées implémentées; le Reader multi-pass dédié ne l’est pas. Grant: submitted / under review / not awarded; €50,000 planning only; budget change: none.
