...
# Проект, грант и governance

physical L3 != strict Canon; retrieval score != evidence; model output != source truth; migration proof != claim proof. PostgreSQL остаётся `active=false`.

NLnet: submitted / under review / not awarded. Около €50,000 — только planning, не approved budget; budget change: none.
...
RC-1/RC-2 — существующий pre-agreement baseline. Dedicated multi-pass Reader остаётся будущей отдельной работой. Юридическая/GDPR/security и native-speaker certification не заявляются.
