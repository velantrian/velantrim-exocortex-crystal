...
def main() -> int:
    ...
    for locale in LOCALES:
        ...
        for name in FILES:
            ...
            for marker in (f"translation-source: {source_doc}@", f"d3-locale: {locale}", "d3-boundary: physical-l3-not-strict-canon", "d3-boundary: public-query-read-only", "d3-boundary: postgresql-active=false", "active=false"):
                if marker not in text: errors.append(f"{relative}: {marker}")
            if locale == "ru":
                for marker in (f"translation-source: {source_doc}@{SOURCE}", "translation-status: CURRENT", "d3-reader: rc5-relation-candidates-implemented", "RC-5", "coverage != comprehension proof", "pass completion != comprehension proof", "EXTRACTED_PROPOSITION != verified fact", "Reader candidate != admitted evidence", "contradiction candidate != confirmed contradiction"):
                    if marker not in text: errors.append(f"{relative}: {marker}")
    ...
    for marker in (f"D3 source checkpoint:** `main@{SOURCE}`", "D3 Reader-dependent detail translations are `CURRENT` in Russian", "eight other supported locales are `REFRESH_NEEDED`"):
