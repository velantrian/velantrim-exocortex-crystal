...
def main() -> int:
    ...
    if docs.get("d1_source_checkpoint") != SOURCE:
        errors.append("manifest: D1 source checkpoint")
    if docs.get("d1_current_locales") != ["ru"]:
        errors.append("manifest: D1 current locales")
    if docs.get("d1_refresh_needed_locales") != list(REFRESH):
        errors.append("manifest: D1 refresh locales")
    ...
    for locale in LOCALES:
        ...
        for name in ("STATUS.md", "IMPLEMENTATION_STATUS.md"):
            ...
            if f"translation-source: {source_doc}@" not in text:
                errors.append(f"{relative}: source marker")
        ...
        quick = (ROOT / f"docs/{locale}/QUICKSTART.md").read_text(encoding="utf-8")
        if "translation-status: CURRENT" not in quick:
            errors.append(f"docs/{locale}/QUICKSTART.md: not CURRENT")
