"""Validate mixed D1 localization freshness after French parity refresh."""
...
def main() -> int:
    ...
    for ok, label in (
        ...
        (documentation.get("latest_translation_refresh_issue") == 414, "latest refresh issue"),
    ...
    for marker in (
        ...
        "D1 Reader-dependent detail translations are `CURRENT` in German, French and Russian",
    ...
    for marker in (
        "German, French and Russian Reader-dependent public/detail documentation is refreshed",
        "six other localized root README files and Reader-dependent detail packs",
        "reader_core_rc5_relation_candidates    = true",
    ):
    ...
    if errors:
        ...
        for error in errors:
            print(f"  - {error}")
        return 1
    print("D1 translation status consistent: German + French + Russian CURRENT; 6 locales REFRESH_NEEDED")
    return 0
