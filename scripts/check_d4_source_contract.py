...
def main() -> int:
    ...
    required = {
        "docs/PROJECT_GRANT_AND_GOVERNANCE.md": (
            "proposal: submitted", "review: in progress", "award: not awarded", "budget change: none",
            "€50,000", "active=false", "RC-9", "dedicated_reader_core=false", "retrieval match",
            "candidate discovery", "candidate adjudication",
        ),
        "docs/GLOSSARY.md": (
            "physical L3", "strict Canon", "active=false", "funded delta", "submitted / under review / not awarded",
            "Reader Core RC-1", "Reader Core RC-2", "Reader Core RC-3", "Reader Core RC-4", "Reader Core RC-5",
            "EXTRACTED_PROPOSITION", "source owner", "POSSIBLE_CONTRADICTION", "EXCEPTION", "QUALIFICATION", "TENSION",
            "dedicated/full Reader Core",
        ),
        "docs/GRANT_NLNET_SCOPE.md": (
            "submitted / under review / not awarded", "budget change", "RC-9", "dedicated_reader_core",
            "existing pre-agreement baseline", "cannot later be rebilled", "Recall@5", "Precision@5",
            "LEXICAL_BASELINE_EXPOSES_MEASURED_GAP", "candidate discovery", "candidate adjudication",
        ),
        "ROADMAP.md": (
            "submitted / under review / not awarded", "reader_core_rc1_skeleton", "reader_core_rc7_cross_document_links",
            "dedicated_reader_core", "RC-9", "LEXICAL_BASELINE_EXPOSES_MEASURED_GAP", "candidate discovery", "candidate adjudication",
        ),
        "GOVERNANCE.md": (
            "physical l3 != strict canon", "public query surfaces remain read-only",
            "postgresql/pgvector remains inactive with `active=false`", "nlnet proposal: submitted / under review / not awarded",
        ...
        "CONTRIBUTING.md": (
            ...
            "submitted / under review / not awarded", "anything merged before a funding agreement is existing baseline",
        ),
    }
    ...
    require("baseline/delta matrix", supporting.get("docs/grants/baseline-funded-delta-matrix.md", ""), (
        "no award", "cannot be counted again", "active=false", "Reader RC-1", "Reader RC-7", "Reader RC-8", "Reader RC-9",
        "dedicated_reader_core", "Recall@5", "Precision@5", "LEXICAL_BASELINE_EXPOSES_MEASURED_GAP", "candidate discovery", "candidate adjudication",
    ), errors)
    require("funding use plan", supporting.get("docs/grants/funding-use-plan.md", ""), (
        "submitted / under review / not awarded", "€50,000", "not an approved budget", "RC-9", "cannot be paid for again",
        "LEXICAL_BASELINE_EXPOSES_MEASURED_GAP",
    ), errors)
    require("reviewer Q&A", supporting.get("docs/grants/reviewer-qa.md", ""), (
        "RC-9", "Recall@5", "Precision@5", "not presented as “94% accuracy”", "submitted / under review / not awarded",
        "dedicated_reader_core=false", "candidate discovery", "candidate adjudication",
    ), errors)

    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    require("root README", readme, (
        "Current implemented Reader retrieval baseline", "RC-9 deterministic lexical PRE-ADMISSION candidate discovery",
        "Recall@5", "Precision@5", "MRR", "LEXICAL_BASELINE_EXPOSES_MEASURED_GAP", "Reviewer validation",
        "submitted / under review / not awarded", "dedicated_reader_core=false", "candidate discovery", "candidate adjudication",
    ), errors)
    ...
    forbid("root README", readme, (
        "94% accuracy", "semantic understanding is implemented", "automatic truth verification is implemented",
        "Crystal implements automatic truth verification", "grant status: awarded",
    ), errors)
    ...
    require("AI current state", state, (
        SOURCE, "reader_core_rc1_skeleton", "reader_core_rc7_cross_document_links", "dedicated_reader_core",
        "RC-9", "LEXICAL_BASELINE_EXPOSES_MEASURED_GAP", "NLnet remains submitted / under review / not awarded",
    ), errors)
    ...
    require("CI workflow", workflow, ("Validate English D4 source contract", "python scripts/check_d4_source_contract.py"), errors)
    ...
    print("English D4 project/grant/governance truth consistent with post-RC-9 public baseline; " f"retained translation source={SOURCE}")
    return 0
