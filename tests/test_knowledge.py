...
def test_extract_json_ignores_unknown_dict_and_empty_string():
    # dict without "claim"/"claims" → no records; empty string → no records
    assert knowledge.extract_claims('{"meta": 1}', "json") == []
    assert knowledge.extract_claims('""', "json") == []
    assert knowledge.extract_claims('123', "json") == []
...
def test_extract_csv_bad_number_is_skipped():
    out = knowledge.extract_claims(
        "claim,confidence,claim_type\nGold is a metal,notanum,WORLD_FACT\n", "csv")
    assert out == [{"claim": "Gold is a metal", "claim_type": "WORLD_FACT"}]
...
def test_ingest_claims_skips_blank_records():
    rep = knowledge.ingest_claims(
        [{"claim": "   "}, {"claim": "Iron is a metal"}], source="s")
...
def test_ingest_file_explicit_fmt_override(tmp_path):
    p = tmp_path / "kb.dat"
    p.write_text("Saturn has rings\n", encoding="utf-8")
    rep = knowledge.ingest_file(str(p), fmt="txt", source="planets")
    assert rep["accepted"] == 1 and rep["source"] == "planets"
