...
A curator decision changes authoritative L1 state, writes accountable audit
proof and may require side effects in the physically separate L3 graph. SQLite
and L3 do not share a transaction, so this module commits the decision, audit
entry and an idempotent projection command in one SQLite transaction. L3 is
then updated synchronously when possible and retried from the durable command
when it is not.

The journal is content-light: it stores identifiers, curator-provided reason /
note text, policy outcome metadata and projection instructions, never claim or
source text.
"""
...
def make_decision_id(
    *,
    ...
    """Return a stable idempotency key for one observed decision snapshot."""
    body = {
        "event": event,
        "fact_id": fact_id,
...
def _append_audit_event_conn(
    ...
    *,
    ...
    """Append through the existing audit contract using the caller transaction."""
    ...
    entry_hash = audit._entry_hash(
        seq, ts, event, fact_id, detail_json, prev_hash
    )
...
def _read_metadata(row) -> dict[str, Any]:
    raw = crypto.decrypt(row["metadata"])
...
def stage_review_decision(
    *,
    ...
    """Commit state, audit proof and projection intent as one SQLite unit.

    Target transitions are mappings containing ``fact_id``,
    ``expected_revision``, ``expected_state`` and ``path``. When
    ``allow_partial_targets`` is true, an existing unrestricted target that
    lost an optimistic state/revision race is preserved and reported as a
    partial target. Missing, erased or restricted targets still fail closed.
    """
    ...
    def _write() -> dict[str, Any]:
        with memory._db() as conn:
            ...
            if candidate["epistemic_state"] != expected_state:
                raise DecisionConflict(
                    "ESM CAS conflict: candidate state changed concurrently"
                )
            if int(candidate["revision"]) != int(expected_revision):
                raise DecisionConflict(
                    "ESM CAS conflict: candidate revision changed concurrently"
                )
            final_candidate_state = _validate_path(
                candidate["epistemic_state"], candidate_path
            )
            ...
            for spec in target_specs:
                ...
                if target is None:
                    raise DecisionConflict(
                        f"conflict target {target_id!r} no longer exists"
                    )
                if target["restricted"]:
                    raise DecisionConflict(
                        f"conflict target {target_id!r} is restricted"
                    )
                matches = (
                ...
                if not matches:
                    if allow_partial_targets:
                        partial_target_ids.append(str(target_id))
                        continue
                    raise DecisionConflict(
                        f"conflict target {target_id!r} changed concurrently"
                    )
            ...
            for spec, target in applicable_targets:
                ...
                for new_state in tuple(spec.get("path") or ()):
                    cur = conn.execute(
                        "UPDATE facts SET epistemic_state = ?, "
                        "revision = revision + 1, updated_at = ? "
            ...
            projection = (
                dict(projection_builder(partial_tuple))
            ...
            payload = {
                "version": 1,
                "audit": {
                    "seq": audit_receipt["seq"],
                    "entry_hash": audit_receipt["entry_hash"],
                },
            ...
            conn.execute(
                ...
                "(decision_id, fact_id, event, payload, projection_status, "
    ...
    try:
        with _DECISION_LOCK, memory._FACTS_WRITE_LOCK:
            result = memory.call_with_lock_retry(_write)
...
def projection_health() -> dict[str, Any]:
    """Content-light operator status; never returns claims, sources or payloads."""
    with memory._db() as conn:
        _ensure_schema(conn)
        rows = conn.execute(
    ...
    return {"counts": counts, "attention": failures}
