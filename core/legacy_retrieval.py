...
class LegacyCandidates(list[dict[str, Any]]):
    """List-compatible result carrying bounded-work metadata even when empty."""
...
def _bounded_nodes(graph, candidate_limit: int) -> list[dict[str, Any]]:
    ...
    if isinstance(graph, MockL3Graph):
        ids = heapq.nsmallest(candidate_limit, graph._nodes)
...
def bounded_legacy_retrieve(
    ...
    *,
    ...
    limit = legacy_candidate_limit(candidate_limit)
    # Unsupported and malformed adapters must never masquerade as an ordinary
    # empty result. They receive the same stable reindex-required contract.
    if not isinstance(graph, (MockL3Graph, SqliteL3Graph)):
        raise LegacyRetrievalUnavailable(
            f"{type(graph).__name__} cannot provide bounded legacy retrieval; "
            "run an explicit embedding reindex before public queries"
        )
    ...
    if not query_tokens:
        return LegacyCandidates((), examined=examined, candidate_limit=limit)
    ...
    return LegacyCandidates(
        ranked[:k], examined=examined, candidate_limit=limit
    )
